...
- Name: home.spec.ts >> Home page >> empty state message appears when search has no matches
- Location: e2e/home.spec.ts:112:7
...
> 116 |     await search.fill("xyzzy_no_recipe_matches_this");
  ...
  206 |       .allInnerTexts();
  207 | 
  208 |     const sorted = [...titles].sort((a, b) =>
  209 |       a.toLowerCase().localeCompare(b.toLowerCase())
  210 |     );
  211 |     expect(titles).toEqual(sorted);
  212 |   });
  213 | 
  214 |   test("sort by Date (newest) is selectable", async ({ page }) => {
  215 |     const select = page.locator('[class*="MuiSelect-select"]');
  216 |     await select.click();
```